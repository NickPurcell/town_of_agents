...
export class OpenRouterService implements LLMService {
  ...
  async generate(
    ...
    try {
      ...
      console.error('\n' + '='.repeat(80));
      console.error('OPENROUTER API ERROR - REQUEST FAILED');
  ...
  async *generateStream(
    ...
    try {
      ...
      console.log(`[TIMING] OpenRouter: Creating stream for ${model}...`);
      ...
      console.log(`[TIMING] OpenRouter: Stream created after ${Date.now() - streamStartTime}ms`);

      let firstChunkLogged = false;
      for await (const chunk of stream) {
        if (!firstChunkLogged) {
          firstChunkLogged = true;
          console.log(`[TIMING] OpenRouter: First chunk after ${Date.now() - streamStartTime}ms`);
        }
      ...
      console.error('\n' + '='.repeat(80));
      console.error('OPENROUTER API ERROR - STREAMING REQUEST FAILED');
      console.error('='.repeat(80));
      console.error('Model:', model);
      console.error('Error:', error);
      console.error('='.repeat(80) + '\n');
